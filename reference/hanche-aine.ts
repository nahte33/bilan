/* =============================================================================
   ...
   COUVERTURE : 4 quadrants
     ...
     + couche DRAPEAUX_ROUGES transversale (fracture de fatigue du col, hernie
       vraie, avulsion apophysaire, épiphysiolyse, infection, tumeur, origine
       lombaire/radiculaire, cause intrapelvienne).

   CADRE DE RÉFÉRENCE :
     - Accord de Doha sur les douleurs inguinales de l'athlète (Weir et al.,
       Br J Sports Med 2015;49:768-774) : 3 catégories — entités cliniques
       définies (adducteurs / iliopsoas / inguinal / pubien), douleur liée à
       la hanche, autres causes.
   ...
   LÉGENDE DES POIDS (PROVISOIRES — à recalibrer cliniquement) :
       ...
       -1 à -2 argument CONTRE l'hypothèse
   ⚠️ Ce ne sont PAS des probabilités validées. Les Se/Sp indiquées varient
      fortement d'une étude à l'autre (voir champ "precision") : à confirmer.
   ============================================================================= */

// ---------- Types (la "forme" des données) ----------
type Condition = Record<string, string | boolean | string[]>;
interface Option { valeur: string; label: string; }
interface Question {
  id: string;
  texte: string;
  type: "choix_unique" | "choix_multiple" | "oui_non";
  options?: Option[];
  condition?: Condition;     // ne s'affiche que si la condition est vraie
  aide?: string;
}
interface Indice { si: Condition; poids: number; source?: string; }
interface Hypothese { id: string; nom: string; quadrant: string; indices: Indice[]; }
interface Test {
  id: string;
  nom: string;
  technique?: string;
  discrimine: string[];      // ids des hypothèses que le test renforce/infirme
  precision?: string;        // Se/Sp + interprétation (rule-in / rule-out)
  source?: string;
}
interface DrapeauRouge {
  id: string;
  libelle: string;
  si: Condition;
  conduite: string;
...
// ---------- Seuils des bandes qualitatives (à recalibrer) ----------
export const BANDES = {
  forte: 4,    // score >= 4 -> "cohérence forte"
  moderee: 2,  // score 2-3  -> "cohérence modérée" ; 1 -> "faible" ; <=0 -> écartée
};
...
// =============================================================================
export const HYPOTHESES: Hypothese[] = [
  {
    id: "adducteurs",
...
export const TESTS: Test[] = [
  ...
  },
  {
    ...
    discrimine: [], // n'oriente pas une entité de Doha : sert le drapeau rouge fracture
...
// =============================================================================
export const DRAPEAUX_ROUGES: DrapeauRouge[] = [
  {
